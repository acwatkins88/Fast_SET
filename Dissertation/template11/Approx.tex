...
%
%\begin{algorithm}
%	\caption{Determine Output} 
%	\begin{algorithmic} [1]
%		\STATE Initialize output with truth table
%		\FOR {each time $i$}
%			\FOR {each transistor $j$}
%				\STATE Get $I_{m_{i,j}}$ from LUT using $i-1$ values
%			\ENDFOR
%			\FOR {each intermediate node $V_n$}
%				\STATE $I_m = \frac{C_{m1}(V_{n_i} - V_{n_{i-1}})}{t_{step}}$
%				\STATE $V_{n_i} = \frac{(I_{D1} - I_{D2})t_{step}}{C_{m1}} + V_{n_{i-1}}$
%			\ENDFOR
%			\IF {$j$ part of transistor stack}
%				\STATE $I_p = avg(I_{m_{i,j}})$
%			\ELSE
%				\STATE $I_p = \sum I_{m_{i,j}}$
%			\ENDIF
%			\STATE $\Delta V_{out} = \frac{(I_{pu}-I_{pd})t_{step} + \sum_{i-1}^{N_u} \Delta V_{G_i}C_{mu_i} + \sum_{i=1}^{N_d} \Delta V_{G_i}C_{md_i}}{C_L + \sum_{i=1}^{N_u}C_{mu_i} + \sum_{i=1}^{N_d}C_{md_i}}$
%		\ENDFOR		
%	\end{algorithmic}
%\end{algorithm}
